...
1. **メニューバー ＆ アクション**:
   - **Pedagogue メニュー**: ℹ️ Pedagogue について (バージョン・仕様準拠・OSSライセンス)
   ...
   - **ヘルプ**: 📖 ヘルプ ＆ 仕様リファレンス (右ドロワー展開), ℹ️ MDR XAFS DB について (仕様説明・公式リンク)
2. **ドラッグ＆ドロップ（D&D）対応**:
...
3. **衝突（コンフリクト）判定＆一括マージダイアログ**:
   ...
   - 操作ボタン順序: **左:「維持（既存優先）」/ 右:「上書き（ファイル優先）」** で統一。
...
8. **ヘルプドロワー・スキーマリファレンス ＆ AI操作支援（Help for AI）**:
   - 右サイドドロワーから「操作ガイド（画面配置順機能解説・Tips）」および「JXSスキーマリファレンス（検索・カテゴリフィルタ・入力連動）」を展開。
   - ドロワー右上の「別タブで開く」から独立したマニュアル（`help.html`）を大画面で参照可能。
   - 不可視DOMタグ（`<!-- Help for AI -->` / `.sr-only`）により、ブラウザサイドバーAIがツールの操作手順や全スキーマを即座に参照可能。
...
| ファイル | 役割・内容 |
| :--- | :--- |
| **`index.html`** | メイン画面（Vuetify 3 構成。メニューバー、Key-Value入力、必須/推奨チップス、衝突モーダル、YAMLプレビュー、ヘルプドロワー） |
| **`pedagogue.js`** | アプリケーションロジック（パスパーサー、サジェスト、ロード＆衝突マージ、ファイルエクスポート、自動整列、ヘルプ連動） |
| **`schema_dictionary.js`** | JXS共通仕様（20230203版）のスキーマ辞書（全Key、型、3段階レベル、Enum、日本語説明、例） |
| **`help.html`** | 操作マニュアル ＆ JXSスキーマリファレンスページ（単体表示・ドロワー埋め込み・AI支援両用） |

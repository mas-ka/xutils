# JXS XAFS Metadata Schema - Comprehensive Spec Pack for Coding Agents
## 日本XAFS研究会 XAFSメタデータ共通仕様 完全仕様ドキュメント（コーディングエージェント用）

本ファイルは、日本XAFS研究会（JXS）XAFSワーキンググループが策定した**「XAFSメタデータ共通仕様（20230203リリース版）」**のすべてのメタデータ定義、ディレクトリ・ツリー階層構造、および登録・利用手引を1つのファイルに統合した**「コーディングエージェント（LLM等）向け完全仕様仕様パッケージ（コンテキストパック）」**です。

自動YAMLジェネレーターや、バリデータプログラム、データベース抽出ツール等の開発エージェントに本ファイルをそのまま読み込ませることで、完璧な文脈理解とコード生成が可能になります。

---

## SECTION 1. メタデータ仕様の概要と基本コンセプト（README）

### 1.1 規格の目的
放射光施設で測定されたXAFS（X線吸収微細構造）測定データを、将来的なマテリアルズ・インフォマティクス（MI）や機械学習、セマンティックウェブでの自動統合・再利用を可能にすることを目的として定義されています。

### 1.2 NIMS MDRスキーマとJXSスキーマの役割分担
* **MDR Schema (`mdr-schema`)**: NIMS（物質・材料研究機構）のデータリポジトリ「MDR」が共通して使用するガワ（外枠）のスキーマ（登録者名、タイトル、公開日、ライセンス等）。
* **JXS Schema (`xafs-schema`)**: メタデータYAMLの内部で定義される、XAFS測定特有の専門スキーマ。測定元素、吸収端、ビームライン、分光結晶定数、検出器（電離箱・SSD）、アンプ系統の仕様などを極めて詳細に網羅します。

### 1.3 表記の基本ルール
* **階層構造（JSONのオブジェクト）の表現**: CSV等で定義されるフラットなKey名において、親子構造は `@` 記号で繋がれます。（例：`@data_info@data_depositor@name` は、`data_info` -> `data_depositor` -> `name` というYAMLの入れ子になります）
* **配列（JSONのリスト）の表現**: カラムが複数の項目を持つ配列である場合、`.` または `@` の下に配列定義を内包して表現します。（例：`@files` はファイル定義のオブジェクトの配列です）

---

## SECTION 2. XAFSメタデータ 階層構造ツリー（TreeView）

YAMLのインデント、構造、各項目のデータ型、および必須/任意の区分を一発で機械認識するための完全ツリーマップです。
...
## SECTION 3. 各大分類（カテゴリ）における詳細Key定義ディクショナリ
